...
export default function EditProfile(){const{t,isRTL}=useI18n();const{profile,loading,saveProfile}=useProfile();const[name,setName]=useState(''),[city,setCity]=useState(''),[neighborhood,setNeighborhood]=useState(''),[err,setErr]=useState(''),[saving,setSaving]=useState(false);
  ...
  const onSave=async()=>{setErr('');const input={full_name:name,city,neighborhood};try{profileSchema.parse(input);setSaving(true);Keyboard.dismiss();await saveProfile({...input,avatar_url:profile?.avatar_url});router.back()}catch(e:any){if(e instanceof z.ZodError)setErr(t('formValidationError'));else setErr(t('profileSaveError'))}finally{setSaving(false)}};
