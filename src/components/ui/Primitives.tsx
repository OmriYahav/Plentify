...
export function Button({title,variant='primary',onPress,style,textStyle,...p}:TouchableOpacityProps&{title:string;variant?:'primary'|'ghost';textStyle?:React.ComponentProps<typeof Text>['style']}){return <TouchableOpacity {...p} style={[s.btn,variant==='ghost'&&s.ghost,p.disabled&&s.disabled,style]} onPress={e=>{Keyboard.dismiss();onPress?.(e)}}><Text style={[s.btnText,variant==='ghost'&&{color:theme.colors.primary},textStyle]}>{title}</Text></TouchableOpacity>}
