...
export function AuthProvider({children}:{children:React.ReactNode}){const[session,setSession]=useState<Session|null>(null);const[loading,setLoading]=useState(true);
  ...
  const signUp=useCallback(async(e:string,p:string,profile?:{full_name:string;city:string})=>{if(!supabase)throw new Error('Supabase is not configured');const email=e.trim().toLowerCase();const options=profile?{data:{full_name:profile.full_name.trim(),city:profile.city.trim()}}:undefined;console.info('[auth] signUp request',{email,profile:profile?{full_name:profile.full_name.trim(),city:profile.city.trim()}:undefined});const{data,error}=await supabase.auth.signUp({email,password:p,options});if(error){console.error('[auth] signUp error',{message:error.message,status:error.status,name:error.name});throw error}console.info('[auth] signUp response',{userId:data.user?.id,hasSession:!!data.session});await ensureProfile(data.session)},[]);
