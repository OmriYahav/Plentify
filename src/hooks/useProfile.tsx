...
export function ProfileProvider({children}:{children:React.ReactNode}){const{t}=useI18n();const{userId,demo}=useAuth();const[profile,setProfile]=useState<Profile|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState('');
  const refreshProfile=useCallback(async()=>{setLoading(true);setError('');try{if(demo){const saved=await AsyncStorage.getItem(STORAGE_KEY);setProfile(saved?{...demoProfile,...JSON.parse(saved)}:demoProfile)}else setProfile(await getProfile(userId) as Profile)}catch(e:any){setError(t('profileLoadError'))}finally{setLoading(false)}},[demo,userId,t]);
