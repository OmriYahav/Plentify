...
export function ProfileProvider({children}:{children:React.ReactNode}){const{t}=useI18n();const{session,userId,demo}=useAuth();const[profile,setProfile]=useState<Profile|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState('');
  const refreshProfile=useCallback(async()=>{setLoading(true);setError('');try{if(!demo&&!session){setProfile(null);return}if(demo){const saved=await AsyncStorage.getItem(STORAGE_KEY);setProfile(saved?{...demoProfile,...JSON.parse(saved)}:demoProfile)}else setProfile(await getProfile(userId) as Profile)}catch(e:any){setError(t('profileLoadError'))}finally{setLoading(false)}},[demo,session,userId,t]);
  useEffect(()=>{refreshProfile()},[refreshProfile]);
  const saveProfile=useCallback(async(patch:Partial<EditableProfile>)=>{setError('');const clean={...patch,full_name:patch.full_name?.trim(),city:patch.city?.trim(),neighborhood:patch.neighborhood?.trim()||null};try{const saved=demo?{...(profile??demoProfile),...clean,updated_at:new Date().toISOString()} as Profile:await updateProfile(userId,clean) as Profile;if(demo)await AsyncStorage.setItem(STORAGE_KEY,JSON.stringify(saved));setProfile(saved);return saved}catch(e:any){setError(t('profileSaveError'));throw e}},[demo,profile,userId,t]);
